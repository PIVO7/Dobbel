# App Store copy — Dobbel! (Français)

Voor het taal-tabblad **French (France)** in App Store Connect (Frans-Canada neemt dit over, of krijgt een kopie). Persona's heten in het Frans: Dodo, Robbie, Professeur Point.

---

## Nom (max 30 tekens)

> Dobbel!

## Sous-titre (max 30 tekens)

> Jeu de dés pour la famille

*(27 tekens)*

## Texte promotionnel (max 170 tekens)

> Lancez, comptez et gagnez ensemble — sur un seul appareil, sans pub ni compte. Trois adversaires pour jouer en solo.

*(118 tekens)*

## Description (max 4000 tekens)

> Dobbel! est le grand classique aux cinq dés, pensé pour les enfants et toute la famille. Lancez jusqu'à trois fois, gardez vos meilleurs dés et choisissez la case la plus maligne de la feuille de score. Qui compte le mieux ?
>
> ENSEMBLE, SUR UN SEUL APPAREIL
> Jouez les uns contre les autres sur un seul iPhone ou iPad. L'appareil passe de main en main, avec un écran de passage tout doux pour que personne ne triche.
>
> OU CONTRE L'ORDINATEUR
> Trois adversaires, chacun avec son caractère : Dodo lance au hasard, Robbie se défend bien et Professeur Point ne rigole pas. Le jeu grandit avec votre enfant.
>
> APPRENDRE SANS LEÇONS
> Les nouveaux joueurs ont droit à une petite visite guidée, la règle du jeu « Comment jouer à Dobbel ? » n'est jamais loin, et la feuille de score fait tous les calculs. Pendant ce temps, ça additionne et ça multiplie — mais nous, on appelle ça jouer.
>
> FAIT POUR LES PETITES MAINS
> De gros boutons, des couleurs joyeuses, un avatar personnel pour chaque joueur, et des dés qu'on lance en secouant l'appareil. Chaque partie se termine par des confettis pour le gagnant.
>
> SÛR ET TRANQUILLE
> Pas de pub, pas de compte, pas besoin d'internet. Tout reste sur votre appareil. Point final.
>
> COMMENCEZ GRATUITEMENT, DÉBLOQUEZ UNE SEULE FOIS
> Gratuitement, vous jouez à deux, contre Robbie, dans le thème classique. La version Famille, en un seul achat, débloque les parties à 3 ou 4 joueurs, les trois adversaires, les thèmes Bonbons, Océan et Nuit, et les statistiques par joueur — avec records et séries de victoires. Un achat pour toute la famille, via le partage familial. Le tout derrière un contrôle parental, pour que les petits doigts n'achètent rien par accident.
>
> Attrapez les dés. Dobbel !

*(± 1700 tekens)*

## Mots-clés (max 100 tekens, komma's, geen spaties)

> yahtzee,enfants,société,calcul,ensemble,apprendre,chiffres,classique,amis

*(75 tekens — « dés », « jeu » en « famille » staan al in de ondertitel)*

⚠️ Zelfde kanttekening: "yahtzee" is een Hasbro-merk; bij een afkeuring vervangen door bv. `jeudedés`.

## Achat intégré

| Veld | Waarde |
|---|---|
| Display name (max 30) | Version Famille |
| Description (max 45) | 3-4 joueurs, thèmes et statistiques |

## Nouveautés (eerste release)

> Bienvenue dans Dobbel ! La toute première version : jouez en famille sur un seul appareil ou contre l'ordinateur.
