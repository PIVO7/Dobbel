...
final class ProfileStore {
    ...
    private let logger = Logger(subsystem: "com.pivo7.dobbel", category: "profielen")

    // De bestandsnaam houdt de oude "yahtzee"-prefix: zo houden bestaande
    // installaties hun profielen na de hernoeming naar Dobbel.
    ...
    func recordGameResult(players: [GamePlayer], winnerProfileIDs: [UUID]) {
        for player in players where !player.isComputer {
            ...
            profiles[index].dobbelCount +=
                (card.scores[.dobbel] == DobbelScorer.dobbelPoints ? 1 : 0)
                + card.dobbelBonusTotal / DobbelScorer.dobbelBonusPoints
    ...
    private func load() {
        ...
        do {
            ...
            // Geen dialoog voor een kind, wel een spoor voor de ontwikkelaar.
