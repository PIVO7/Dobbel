...
struct ComputerAI {
    func decide(
        ...
        // If we already have a strong scoreable Dobbel / large straight, take it.
        if DobbelScorer.isDobbel(values), open.contains(.dobbel) || DobbelScorer.canUseJoker(dice: values, scorecard: scorecard) {
            return ComputerDecision(
        ...
        let large = DobbelScorer.score(category: .largeStraight, dice: values)
        ...
        if rollsRemaining == 1 {
            ...
            let expected = best.map { DobbelScorer.pointsForPlacing(category: $0, dice: values, scorecard: scorecard).score } ?? 0
    ...
    private func decideEasy(
        ...
        if DobbelScorer.isDobbel(values), open.contains(.dobbel) {
            return ComputerDecision(
                holdMask: Array(repeating: true, count: 5),
                shouldScore: true,
                category: .dobbel
        ...
        if rollsRemaining == 0 {
            let category = open.max { lhs, rhs in
                let l = DobbelScorer.pointsForPlacing(category: lhs, dice: values, scorecard: scorecard).score
                let r = DobbelScorer.pointsForPlacing(category: rhs, dice: values, scorecard: scorecard).score
    ...
    private func utility(
        ...
        let result = DobbelScorer.pointsForPlacing(category: category, dice: values, scorecard: scorecard)
        var score = Double(result.score + result.dobbelBonus)
        ...
        if level == .hard, category.isUpper, result.score > 0,
           DobbelScorer.upperBonusStillPossible(scorecard: scorecard) {
            score += Double(result.score) * Double(DobbelScorer.upperBonusPoints) / Double(DobbelScorer.upperBonusThreshold)
        }

        // Avoid wasting Dobbel box on zero when alternatives exist.
        if category == .dobbel, result.score == 0 {
            score -= 40
        }
        ...
        if result.score == 0 {
            switch category {
            case .dobbel, .largeStraight, .smallStraight, .fullHouse:
                score -= 15
            default:
                score -= 3
            }
