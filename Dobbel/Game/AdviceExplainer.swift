...
enum AdviceExplainer {
    static func explain(category: ScoreCategory, dice: [Int], scorecard: Scorecard) -> String {
        let score = DobbelScorer.pointsForPlacing(
            category: category,
            dice: dice,
            scorecard: scorecard
        ).score
        ...
        case .ones, .twos, .threes, .fours, .fives, .sixes:
            ...
            if DobbelScorer.upperBonusStillPossible(scorecard: scorecard) {
        ...
        case .fullHouse:
            ...
            if DobbelScorer.canUseJoker(dice: dice, scorecard: scorecard) {
