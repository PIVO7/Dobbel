...
enum RollPhrase {
    ...
    static func describe(_ dice: [Int]) -> String {
        ...
        if DobbelScorer.score(category: .largeStraight, dice: dice) > 0 { return String(localized: "Grote straat!") }
        if DobbelScorer.score(category: .smallStraight, dice: dice) > 0 { return String(localized: "Kleine straat!") }
        if DobbelScorer.score(category: .fullHouse, dice: dice) > 0 { return String(localized: "Vol huis!") }

        let tally = DobbelScorer.counts(for: dice)
