...
struct GameResultOverlay: View {
    ...
    private var shownCount: Int { animatesIn ? countedValue : winnerScore }
    ...
    private var winnerScore: Int {
        winner?.scorecard.total ?? 0
    }

    /// De banner noemt de winnaar bij naam; een kaal "Gewonnen!" leest
    /// alsof jíj won, ook wanneer de computer er met de punten vandoor ging.
        ...
        VStack(spacing: m.gutter * 0.85) {
            ...
            if let winner {
                if winner.isComputer {
                    ...
                    TallyTiles(value: shownCount, label: String(localized: "punten"))
                        .opacity(bannerIn ? 1 : 0)
                        .accessibilityLabel(String(localized: "gewonnen met \(winnerScore) punten"))
                }
            } else {
                Text(message)
                    .font(AppTheme.rounded(m.bodySize + 2, .bold))
            ...
            if isNewRecord {
                // In caps mét kerning, net als de winnaarsbanner: alle
                // feestmomenten spreken dezelfde taal.
    ...
    private func podiumColumn(_ entry: (player: GamePlayer, rank: Int, score: Int)) -> some View {
        ...
        return VStack(spacing: m.gutter * 0.6) {
            ...
            PodiumBlock(
                ...
                height: height,
                statText: String(localized: "\(entry.score) punten")
            )
        ...
        .accessibilityLabel(
            isFirst
                ? String(localized: "\(entry.player.name), \(entry.score) punten, winnaar")
                : String(localized: "\(entry.player.name), \(entry.score) punten")
        )
    }
