...
/// De statistiekenpagina van één profiel: cijfers, het grafiekje van de
...
struct ProfileStatsView: View {
    ...
    private func numbers(for profile: PlayerProfile) -> some View {
        let average = profile.gamesPlayed > 0
    ...
    private func historyChart(for profile: PlayerProfile) -> some View {
        ...
        let top = max(games.map(\.score).max() ?? 1, 1)
                    ...
                    ForEach(Array(games.enumerated()), id: \.offset) { index, game in
                        VStack(spacing: 4) {
                            ...
                            RoundedRectangle(cornerRadius: m.cellCorner * 0.7, style: .continuous)
                                ...
                                .frame(height: max(barMaxHeight * CGFloat(game.score) / CGFloat(top), 10))
                        }
                        .frame(maxWidth: .infinity)
                        .accessibilityElement(children: .ignore)
                        .accessibilityLabel(
                            game.won
                                ? String(localized: "Potje \(index + 1): \(game.score) punten, gewonnen")
                                : String(localized: "Potje \(index + 1): \(game.score) punten")
                        )
                    }
                }
                .padding(m.gutter)
    ...
    private var badgeColumns: [GridItem] {
        Array(repeating: GridItem(.flexible(), spacing: m.cellGap * 2), count: 3)
    }
